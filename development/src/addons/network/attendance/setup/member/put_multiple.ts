...
export async function PUT_AttendanceUpdateScheduleMemberMultiple(meetingEventId: Number, ): Promise<any> {

  const _get_cookie = getUserLoginInfoCookie();
  // console.log({_get_cookie});

  const url = CONSTANTS.URLS.AKWAABA_API_BASE_URL + 'attendance/meeting-event/member/multiple/' + meetingEventId,
    formContainer: HTMLFormElement = document.querySelector('[make-general-posts="add-schedule-member"]'),
    formData = new FormData(formContainer);

  return Swal.fire({
    title: 'Update Setup Member(s)?',
