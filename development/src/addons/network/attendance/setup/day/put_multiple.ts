...
export async function PUT_AttendanceUpdayScheduleDayMultiple(meetingEventId: Number, ): Promise<any> {

  const _get_cookie = getUserLoginInfoCookie();
  // console.log({_get_cookie});

  const url = CONSTANTS.URLS.AKWAABA_API_BASE_URL + 'attendance/meeting-event/day/multiple/' + meetingEventId,
    formContainer: HTMLFormElement = document.querySelector('[make-general-posts="add-schedule-day"]'),
    formData = new FormData(formContainer);

  return Swal.fire({
    title: 'Upday Setup Day(s)?',
