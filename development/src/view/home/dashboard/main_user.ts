...
@customElement("pdb-dashboard-main-user")
    ...
    const metrics = this.dashboardMetrics.map(element => {
      return html`
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 grid-flow-row gap-4 pb-2">
          ${this.metric("All Members", "fa-users", "bg-midnight-bloom", element.statistics.allMembers)}
          ${this.metric("All Males", "fa-male", "bg-midnight-bloom", element.statistics.allMales)}
          ${this.metric("All Females", "fa-female", "bg-midnight-bloom", element.statistics.allFemales)}
          ${this.metric("All Organizations", "fa-building", "bg-midnight-bloom", element.statistics.allOrganizations)}
          ${this.metric("All Admins", "fa-id-badge", "bg-midnight-bloom", element.statistics.allAdmins)}
          ${this.metric("All Archived", "fa-archive", "bg-midnight-bloom", element.statistics.allArchived)}
        </div>
        ${this.headers(element.statistics.currentBranch.branch.name + " Branch Metrics")}
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 grid-flow-row gap-4 pb-2">
          ${this.metric("All Members", "fa-users", "bg-night-sky", element.statistics.currentBranch.allMembers)}
          ${this.metric("All Males", "fa-male", "bg-night-sky", element.statistics.currentBranch.allMales)}
          ${this.metric("All Females", "fa-female", "bg-night-sky", element.statistics.currentBranch.allFemales)}
          ${this.metric("All Organizations", "fa-building", "bg-night-sky", element.statistics.currentBranch.allOrganizations)}
          ${this.metric("All Admins", "fa-id-badge", "bg-night-sky", element.statistics.currentBranch.allAdmins)}
          ${this.metric("All Archived", "fa-archive", "bg-night-sky", element.statistics.currentBranch.allArchived)}
        </div>
        ${this.headers("Subscription Info")}
        <div class="grid grid-cols-1 md:grid-cols-2 grid-flow-row gap-4 pb-5">
          ${this.metric("Subscription Amount", "pe-7s-cash py-2", "bg-vicious-stance", element.statistics.subscriptionAmount, "p-3")}
