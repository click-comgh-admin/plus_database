...
@customElement("membership-groupings-bulk_assignment-member_categories-view")
export class MembershipGroupingsBulkAssignmentMemberCategoriesView extends LitElement {
  constructor() { super(); }
  ...
  @property({ type: String })
  private datatablePathUrl: string = "members/groupings/bulk-assignment/get-member-category";
  ...
  private removeFromList(data: any, type: any, memberInfo: MembershipUserModel) {
    ...
    return `
      <div class="flex flex-col whitespace-normal justify-between content-between">
        <mwc-button class="button danger" outlined delete-this-item="${MEMBER_ID}"
          delete-item-member-type="${memberInfo.accountType}">
  ...
  async deleteMemberFromList(e: Event) {
    ...
    // @ts-ignore
    const MemberID = Number(e.currentTarget.getAttribute('delete-this-item'));
    // @ts-ignore
    const AccountType = Number(e.currentTarget.getAttribute('delete-item-member-type'));

    if (AccountType == 1) {
      await DELETE_MembershipUsers(MemberID);
    } else {
      await DELETE_MembershipOrganizationUsers(MemberID);
    }
  }
