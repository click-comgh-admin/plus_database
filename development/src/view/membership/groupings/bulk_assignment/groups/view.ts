...
import { DELETE_MemberGroupingsGroupMember } from '@@addons/network/members/groupings/group/delete_member';
...
@customElement("membership-groupings-bulk_assignment-groups-view")
export class MembershipGroupingsBulkAssignmentMemberGroupsView extends LitElement {
  constructor() { super(); }
  ...
  @property({ type: String })
  private datatablePathUrl: string = "members/groupings/bulk-assignment/get-groups";
  ...
  private removeFromList(data: any, type: any, memberInfo: MembershipUserModel) {
    ...
    const _urlQueryParams = urlQueryParams(),
      groupId = 'groupId' in _urlQueryParams ? Number(_urlQueryParams['groupId']) : 0;

    return `
      <div class="flex flex-col whitespace-normal justify-between content-between">
        <mwc-button class="button warning" outlined delete-this-item="${groupId}"
    ...
    const groupField = html`
      <div class="col-xl-4 col-md-6">
        <div class="form-input-container !block">
          ...
          <select-input name="${filterNameId_group}" class="w-full" id="${filterNameId_group}" label="Select User Group(s)" .options="${groups}"
            outlined required >
          </select-input>
        </div>
      </div>`;
  ...
  async deleteMemberFromList(e: Event) {
    ...
    // @ts-ignore
    const groupId = Number(e.currentTarget.getAttribute('delete-this-item'));
    // @ts-ignore
    const memberId = Number(e.currentTarget.getAttribute('delete-item-member-id'));

    await DELETE_MemberGroupingsGroupMember(groupId, memberId);
  }
