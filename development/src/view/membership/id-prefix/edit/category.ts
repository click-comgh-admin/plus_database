...
import { PUT_MemberCategoryIdPrefix } from '@@addons/network/members/id_prefix/category/put';
import { MembershipMemberCategoryIDPrefixModel } from '@@addons/interfaces/members/id_prefix/category';
...
@customElement("id-category-prefix-edit")
export class IdCategoryPrefixEdit extends LitElement {
  constructor() { super(); }
  ...
  @property({ type: Array })
  public memberCategories: GroupingsMemberCategoryModel[] = [];
                    ...
                    <mwc-select name="memberCategoryId" class="w-full" id="memberCategoryId" setSelectedMemberCategoryId label="Select Member Category" outlined required>
                      ${this.memberCategories.map((value) => {
                        if (this.prefixEdit.length === 0) {
                          ...
                          if (value.id === this.prefixEdit[0].memberCategoryID.id) {
  ...
  async submitForm(e: PointerEvent) {
    ...
    await PUT_MemberCategoryIdPrefix(this.prefixId);
  }
