...
import { GET_AttendanceDownloadAttendeesExcel } from '@@addons/network/attendance/clocking/download/excel/attendees';
import { GET_AttendanceDownloadAttendeesPdf } from '@@addons/network/attendance/clocking/download/pdf/attendees';
...
@customElement("pdb-attendance-clocking-attendees")
export class PdbAttendanceClockingAttendees extends LitElement {
  constructor() { super(); }
  ...
  @property({ type: String })
  private datatablePathUrl: string = "attendance/meeting-event/attendance/attendees";
  ...
  render() {
    if (this._hasSetup) {
      ...
      this._pageButtonAccess = PageButtonUserAccess(PAGE__IDS.attendees);
      if (!AppSettingsExtraUserAccess({pageId: PAGE__IDS.attendees, viewType: "Both"}, false)) {
  ...
  private renderMemberInfo(data: any, type: any, clockingInfo: MemberClockingInfo_I) {
    // console.log({"clockingInfo-clockingInfo": clockingInfo});
    // @ts-ignore
  ...
  private async downloadAttendeesPdf() {
    const memberType = this._memberType;
    let branchId = 0;
    if (this._activeBranchId !== null) {
      branchId = this._activeBranchId[0].id;
    ...
    const _networkResponse = await GET_AttendanceDownloadAttendeesPdf<any>(URL);
    // console.log({_networkResponse});
  }

  private async downloadAttendeesExcel() {
    const memberType = this._memberType;
    let branchId = 0;
    if (this._activeBranchId !== null) {
      branchId = this._activeBranchId[0].id;
    ...
    const _networkResponse = await GET_AttendanceDownloadAttendeesExcel<any>(URL);
    // console.log({_networkResponse});
  }
