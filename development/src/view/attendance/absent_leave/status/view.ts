...
import { Convert as aalsmConvert } from "@@addons/interfaces/attendance/absent_leave/status";
...
@customElement("attendance-absent-leave-status-view")
export class AttendanceAbsentLeaveStatusView extends LitElement {
  constructor() { super(); }
  ...
  private datatablePathUrl: string = "attendance/absent-leave/status/table";
  ...
  render() {
    if (this._hasSetup) {
      ...
      this._pageButtonAccess = PageButtonUserAccess(PAGE__IDS.absent_leave_status);
      if (!AppSettingsExtraUserAccess({pageId: PAGE__IDS.absent_leave_status, viewType: "View"}, false)) {
  ...
  private renderColOne(data: any, type: any, statusInfo: any) {
    // console.log({statusInfo});
    const StatusInfo = aalsmConvert.toAttendanceAbsentLeaveStatusModel(JSON.stringify(statusInfo));

    return `
      <div class="flex flex-col whitespace-normal justify-between content-between">
        <p class="p-2 shadow-sm my-2 w-full">${StatusInfo.status}</p>
      </div>
    `;
  }

  private renderColTwo(data: any, type: any, statusInfo: any) {
    // console.log({statusInfo});
    const StatusInfo = aalsmConvert.toAttendanceAbsentLeaveStatusModel(JSON.stringify(statusInfo)),
      STATUS_ID = StatusInfo.id,
      STATUS_CODE = base64Encode(String(STATUS_ID), true);
    
    // console.log({"this._pageButtonAccess": this._pageButtonAccess});
    
    const delete_button = this._pageButtonAccess ? `<mwc-button class="button danger" outlined delete-this-item="${STATUS_ID}">
      <mwc-icon class="mr-1">delete_forever</mwc-icon> Delete
    </mwc-button>`: '';
    return `
      <div class="flex flex-col whitespace-normal justify-between content-between">
        ${StatusInfo.clientID === 0? '' : `${delete_button}<link-button isblockcontent="false" aClass="" raised bClass="button warning mr-2" href="${CONSTANTS.URLS.PDB_CLIENT}absent-leave/edit-al-status?view-status=${STATUS_CODE}"
          label="Update">
        </link-button>`}
  ...
  private __dataTable(url: string): DataTables_Settings_I {
    ...
    let dataTable: DataTables_Settings_I = {
      ...
      "columns": [
        {
          data: 'id',
          render: (data: any, type: any, statusInfo: any) => __this.renderColOne(data, type, statusInfo),
          orderable: true
        },
        {
          data: 'status',
          render: (data: any, type: any, statusInfo: any) => __this.renderColTwo(data, type, statusInfo),
          orderable: true
        },
