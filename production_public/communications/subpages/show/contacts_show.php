<?php
    ...
    $get_string = "mailer/one_contact_mail?account_id=$account_id&message_id=".$_GET['show'];
    ...
    <?php die(); }
    $get_string = "mailer/contact_mail_trail?account_id=$account_id&message_id=".$_GET['show'];
                        ...
                        <td>
                            <?php
                                if (@$message['parties'][0]['pdccmc_sender'] == 1) {
                                    ...
                                    $get_string = "one_contact?account_id=$account_id&id=".@$message['parties'][0]['pdccmc_sender_id'];
                                    ...
                                    ?>
                                        <a href="<?= CLIENT_BASE_URL; ?>communication?contacts&contact=<?= $contact['pdccc_code']; ?>" class="btn btn-success btn-sm" type="button">
                                            <?= $contact['pdccc_name']; ?>
                                        </a>
                                    <?php
                                }
                                ...
                                if (@$message['parties'][0]['pdccmc_sender_id'] == $account_id) {
                                    if (!empty($message['parties'])) {
                                        foreach ($message['parties'] as $key => $recipient) {
                                            $get_string = "one_contact?account_id=$account_id&id=".@$recipient['pdccmc_recipient_id'];
                                            $contact = $ccApi->communications($type="clients", $get_string, function($contact)
                                            {
                                                return @$contact[0];
                                            }, $debug = false); ?>
                                            <a href="../communication?contacts&contact=<?= $contact['pdccc_code']; ?>" class="btn btn-info m-1 btn-sm">
                                                <?= $contact['pdccc_name']; ?>
                                            </a>,
                                        <?php }
                                    } else {
    ...
    function delete_btn_alt($message_id, $account_id, $user_id, $class) { ?>
        <button class="<?= $class; ?> d-flex btn-actions-pane-right" style="padding: .25rem .5rem !important"
            ...
            btn-submit-post--url="<?= API_BASE_URL."communications/clients/mailer/delete"; ?>">
...
<div class="container" email-tabs="container-mail">
            ...
            <div class="card-header-title" style="white-space: normal">
                <?php
                    ...
                    <?php } else { 
                        ...
                        ?>
                        <img width="50" height="50" class="rounded-circle my-1 mr-2" alt="Member Image"
    ...
    <div>
        <h5 class="text-center">
            Message Thread
            <a href="contacts?<?= $_SERVER['QUERY_STRING']; ?>&reply" type="button" class="btn btn-sm bg-grow-early text-light font-weight-bold rounded-pill shadow" 
