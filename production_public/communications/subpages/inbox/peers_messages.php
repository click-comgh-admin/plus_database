<?php
    $get_string = "mailer/peers/contact_mail_messages?account_id=$account_id&user_id=$user_id";
...
<ul class="list-group list-group-flush" email-tabs="container-list">
    <?php
        foreach ($messages as $key => $message) { 
            $sender_id = ($message['pdccmc_sender_id'] == $user_id) ? $message['pdccmc_recipient_id']: $message['pdccmc_sender_id'];
            $other_info = $ccApi->user_info($type = "client", $account_id, $all="one_user", $id=$sender_id,
                function ($info) { return @$info[0];
            });
            $thisMsgId = $message['pdccmc_thread_id'] == 0 ? $message['pdccmc_id']: $message['pdccmc_thread_id'];

            $has_attachment = (empty($message['attached']) || empty(@$message['attached'][0])) ? false : true; ?>
            <!-- <pre><?php //print_r($message); ?></pre> -->
            <a class="list-group-item" email-tabs="container-list--item" href="peers?show=<?= $thisMsgId; ?>#<?= $message['pdccmc_id']; ?>">
                <div email-tabs="container-list--item---content">
                    <img width="50" height="50" class="rounded-circle mt-2 shadow shadow-sm" alt="Contact Image"
                        src="<?= FILE_BUCKET_BASE_URL; ?>files/clients/profile-picture/<?= $other_info['pdcu_profile_picture']; ?>">
                    <div email-tabs="container-list--item---content----text">
                        <h6 class="font-weight-bold"> <?= $other_info['pdcu_firstname']." ".$other_info['pdcu_surname']; ?> </h6>
