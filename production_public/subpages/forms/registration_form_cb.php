<form role="form" action="<?= API_BASE_URL; ?>client-membership/add_contact_book" click-com-wizard="form-container" reload_to="<?= CLIENT_BASE_URL; ?>/login">
	<div click-com-wizard="container">
					...
					<?php
						...
						$account_type = $ccApi->fetch_from($get_string=$get_string, function($account_types) {
							...
							foreach ($account_types as $key => $account_type) {
								if ((int)$account_type['pdd_id'] === 12) {//NEW
				...
				<div class="col-lg-6">
					...
					<?php
						$account_type = $ccApi->account_type($all=true, $id=1, function($account_types) {
							$at = '';
							foreach ($account_types as $key => $account_type) {
								if ((int)$account_type['pdd_id'] === 6) {//NEW
									$at .= '<option value="'.$account_type['pdd_id'].'">'.$account_type['pdd_info'].'</option>';
								}
							}
							return @$at;
		...
		<div class="pt-4 pb-0" click-com-wizard-title="Account Info" click-com-wizard="step">
			<h4>Account Info</h4>
			<div class="row">
				<div class="col-lg-6">
					<?php 
