<?php
    $page = "print-reminders";
    $page_url = Current_BaseUrl."$page";

    $_start_date = "start_date";
    $_end_date = "end_date";

    if (isset($_GET[$_start_date])) { ?>
        <script> __set_cookies__("<?= $_start_date; ?>", "<?= $_GET[$_start_date]; ?>", $time = 2); </script>
    <?php }
    if (isset($_GET[$_end_date])) { ?>
        <script> __set_cookies__("<?= $_end_date; ?>", "<?= $_GET[$_end_date]; ?>", $time = 2); </script>
    <?php }

    $fiveDaysBefore = date("Y/m/").((int)date("d")-5);
    $today = date("Y/m/d");
    $start_date = @(isset($_COOKIE[$_start_date])) ? date("Y/m/d", strtotime($_COOKIE[$_start_date])) : $fiveDaysBefore;
    $end_date = @(isset($_COOKIE[$_end_date])) ? date("Y/m/d", strtotime($_COOKIE[$_end_date])) :$today;

    $_by_group_id = "by_group_id";
    $_group_id = "group_id";
    if (isset($_GET[$_group_id])) { 
        $by_group_id = (int)$_GET[$_group_id];
        $this_group_id = ($by_group_id == 0) ? "all": $by_group_id;
        $by_group_id = ($by_group_id == 0) ? 0: 1;
        ?>
        <script> __set_cookies__("<?= $_group_id; ?>", "<?= $this_group_id; ?>", $time = 2); </script>
    <?php } else {
        $by_group_id = @(isset($_COOKIE[$_by_group_id])) ? $_COOKIE[$_by_group_id] : 0;
        $this_group_id = @(isset($_COOKIE[$_group_id])) ? $_COOKIE[$_group_id] : "all"; 
        $by_group_id = ($by_group_id == 0) ? 0: 1;?>
        <script> __set_cookies__("<?= $_by_group_id; ?>", "<?= (strtolower($this_group_id) == "all") ? 0: 1; ?>", $time = 2); </script> <?php
    }

    $_by_reminder_status = "by_reminder_status";
    $_reminder_status = "reminder_status";
    if (isset($_GET[$_reminder_status])) { 
        $by_reminder_status = (int)$_GET[$_reminder_status];
        $this_reminder_status = ($by_reminder_status == 0) ? "all": $by_reminder_status;
        $by_reminder_status = ($by_reminder_status == 0) ? 0: 1;
        ?>
        <script> __set_cookies__("<?= $_reminder_status; ?>", "<?= $this_reminder_status; ?>", $time = 2); </script>
    <?php } else {
        $by_reminder_status = @(isset($_COOKIE[$_by_reminder_status])) ? $_COOKIE[$_by_reminder_status] : 0;
        $this_reminder_status = @(isset($_COOKIE[$_reminder_status])) ? $_COOKIE[$_reminder_status] : "all"; 
        $by_reminder_status = ($by_reminder_status == 0) ? 0: 1; ?>
        <script> __set_cookies__("<?= $_by_reminder_status; ?>", "<?= (strtolower($this_reminder_status) == "all") ? 0: 1; ?>", $time = 2); </script> <?php
    }
?>
<div class="row justify-content-center">
    <div class="col-md-12 col-lg-12">
        <div class="mb-3 card shadow">
            <div class="card-body">
                <div class="row align-items-center justify-content-center">
                    <div class="col-md-12 col-lg-12"></div>
                    <div class="col-md-12 col-lg-6">
                        <?php require_once Current_BaseDir."/forms/filter/date_filter_form.php";?>
                    </div>
                    <div class="col-md-12 col-lg-6">
                        <?php require_once Current_BaseDir."/forms/filter/group_filter_form.php";?>
                    </div>
                    <div class="col-md-12 col-lg-12"></div>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="row justify-content-center">
    <div class="col-md-12 col-lg-12">
        <div class="main-card mb-3 card shadow">
            <div class="card-body px-2">
                <h5 class="card-title">Reminders</h5>
                <div class="table-responsive">
                    <table class="mb-0 table table-striped" pagin_table="office_reminders_print" pt_c_by_date="1" pt_c_user="<?= $user_id; ?>"
                        pt_c_start_date="<?= $start_date; ?>" pt_c_end_date="<?= $end_date; ?>" pt_c_cid="<?= AccountInfoId; ?>" 
                        pt_c_by_reminder_status="<?= $by_reminder_status; ?>" pt_c_reminder_status="<?= $this_reminder_status; ?>" 
                        pt_c_by_group_id="<?= $by_group_id; ?>" pt_c_group_id="<?= $this_group_id; ?>">
                        <thead>
                            <tr>
                                <th>No#</th>
                                <th style="white-space: nowrap;">Subject</th>
                                <th style="white-space: nowrap;">Date</th>
                                <th style="white-space: nowrap;">Time</th>
                                <th style="white-space: nowrap;">Repeat</th>
                                <th style="white-space: nowrap;">Repeat Date 1</th>
                                <th style="white-space: nowrap;"><sapn class="d-none">Repeat Time 1</sapn></th>
                                <th style="white-space: nowrap;"><sapn class="d-none">Repeat Date 2</sapn></th>
                                <th style="white-space: nowrap;">Repeat Time 2</th>
                                <th style="white-space: nowrap;">Repeat Date 3</th>
                                <th style="white-space: nowrap;">Repeat Time 3</th>
                                <th style="white-space: nowrap;">Reminder Status</th>
                                <th style="white-space: nowrap;">Group</th>
                                <th style="white-space: nowrap;">Members</th>
                                <th style="white-space: nowrap;">Other_contacts</th>
                                <th style="white-space: nowrap;"><sapn class="d-none">Medium</sapn></th>
                                <th style="white-space: nowrap;"><sapn class="d-none">Entered By</sapn></th>
                                <th style="white-space: nowrap;"><sapn class="d-none">Entry Date</sapn></th>
                                <th style="white-space: nowrap;"><sapn class="d-none">Updated By</sapn></th>
                                <th style="white-space: nowrap;"><sapn class="d-none">Update Date</sapn></th>
                            </tr>
                        </thead>
                        <tbody>
                            
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>