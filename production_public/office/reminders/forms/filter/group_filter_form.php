<form class="container" role="form" action="<?= $page_url; ?>" method="GET">
    <div class="row align-items-center justify-content-center">
        ...
        <div class="col-lg-6 col-md-6">
            <?php 
                $tittle_placeholder = "Reminder Status";
                $id_name = "reminder_status";
            ?>
            <h6><?= $tittle_placeholder; ?></h6>
            <?php
                $input_value = @(isset($_COOKIE[$id_name])) ? $_COOKIE[$id_name] : "";
                $selectedReminderStatus = isset($_GET[$id_name]) ? $_GET[$id_name] : $input_value;
                $get_string = "office/reminders/reminder_statuses";
                $groupList = $ccApi->fetch_from($get_string=$get_string, function($statuses) use($selectedReminderStatus) {
                    $at = '';
                    if (is_array($statuses)) {
                        foreach ($statuses as $key => $status) {
                            $thisSelected = ((int)$status['pdd_id'] == (int)$selectedReminderStatus) ? "selected" : "";
