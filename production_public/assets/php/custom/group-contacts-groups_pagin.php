<?php
    ...
    $data['user_limit'] = $_GET['user_limit'];
    $data['access_code'] = $_GET['access_code'];

	$curl = curl_init();
	$response = curlCall($base_url."communications/clients/contacts/groups/group-groups-pagin?account_id=".$_GET['acc_id'], $curl, $method="POST", $data);
    // print_r($response);
	$err = curl_error($curl);
    curl_close($curl);
