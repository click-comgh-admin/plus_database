<?php
    ...
    $data['admin_user_id'] = $_GET['admin_user'];
	...
	$curl = curl_init();
	$response = curlCall($base_url."communications/clients/contacts/groups/agent-group-contacts-pagin-admin?account_id=".$_GET['acc_id'], $curl, $method="POST", $data);
    // print_r($response);
	$err = curl_error($curl);
    curl_close($curl);
