<?php
    ...
    $data['by_folder_id'] = $_GET['by_folder_id'];
    $data['folder_id'] = $_GET['folder_id'];
    $data['by_subfolder_id'] = $_GET['by_subfolder_id'];
	...
	$curl = curl_init();
	$response = curlCall($base_url."file-manager/clients/pagin_file-finder", $curl, $method="POST", $data);
    // print_r($response);
	$err = curl_error($curl);
    curl_close($curl);
