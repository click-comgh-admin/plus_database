<?php    
    ...
    $get_string = "clients/files/archived_file?account_id=$account_id&id=".$_GET['id'];
    ...
    function restore_btn($file_id, $account_id, $user_id, $class) { ?>
        <button class="<?= $class; ?>" style="padding: .25rem .5rem !important"
            ...
            btn-submit-post--confirm="Restore File?"
            btn-submit-post--url="<?= API_BASE_URL."file-manager/clients/file/restore"; ?>">
            <span class="pe-7s-folder"></span>
            <span class="d-none d-sm-block">Restore</span>
        </button>
    <?php }    

    $uploadedBy = $ccApi->user_info($type = "client", $account_id = AccountInfoId, $all="one_user", 
    ...
    $archivedBy = $ccApi->user_info($type = "client", $account_id = AccountInfoId, $all="one_user", 
        $id=(isset($file['pdcfmf_archived_by'])? $file['pdcfmf_archived_by']: 0), function ($info) {
        if (empty($info) || (isset($info['error']) && ($info['error'] == 1))) {
            return "Unknown??";
        } else {
            return @$info[0]['pdcu_firstname'] . " " . @$info[0]['pdcu_surname'] . ", / <span class=\"p-2 shadow shadow-lg\">Phone: <a class=\"p-2\" href=\"tel:".$info[0]['pdcu_phone']."\">".$info[0]['pdcu_phone']."</a></span>";
    ...
    $archiveDate = isset($file['pdcfmf_archived_date'])? date("Y-m-d @ H:i A", strtotime($file['pdcfmf_archived_date'])): '-- @ : ';
    $fileInfo = "
        <div class=\"table-responsive\">
            <table class=\"table table-striped table-hover\">
                ...
                <tbody>
                    ...
                    <tr>
                        <th>Archived By<th><td>".$archivedBy."</td>
                    </tr>
                    <tr>
                        <th>Archived On<th><td>".$archiveDate."</td>
                    </tr>
                </tbody>
            </table>
            <table class=\"table table-striped table-hover\">
...
<div class="container-fluid">
    <div class="card p-2 mb-2">
        ...
        <div class="btn-actions-pane-right">
            ...
            <?= restore_btn($file['pdccmc_id'], AccountInfoId, $user_id, "btn btn-sm bg-grow-early text-white font-weight-bold"); ?>
        </div>
    </div>
    <div>
