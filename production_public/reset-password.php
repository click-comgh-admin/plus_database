						...
						<div class="content shadow border-0">
							<?php
								if (isset($_GET['email_token'])) {
									...
									// $email_token = base64_decode($email_token);
									$encryptor = new Encryptor("clients", md5("clients"));
									$email_token = $encryptor->decrypt($email_token);
									$email_token_date = @explode("&+", $email_token);
									$email_token_date = @end($email_token_date);

									$TimeAgo = new TimeAgo(date('y-m-d', strtotime($email_token_date)));
									
									if ($TimeAgo->getDays() > 2) {
										echo "
											<div class='container'>
												<div class='row align-items-center justify-content-center'>
													<div class='col-lg-6 col-md-8'>
														<h4 class='shadow rounded-lg text-danger alert-danger p-1'>
															Sorry, this link has expired. Enter your E-mail Address to continue.
														</h4>
													</div>
												</div>
											</div>
										...
										require_once 'subpages/forms/reset-password_form.php';
									}
								} else {
									require_once 'subpages/forms/forgot-password_form.php';
								}
