<form role="form" class="my-1" action="<?= schoolFees_BaseUrl . "?" . $_SERVER['QUERY_STRING']; ?>" method="GET" enctype="multipart/form-data">
    <div class="row justify-content-center">
        ...
        <div class="col-md-9">
            <div class="row justify-content-center">
                ...
                <div class="col-lg-6 col-md-6">
                    <?php
                        ...
                        $terms = $ccApi->school($get_string, function($terms) use($id_name) {
                            $at = '';
                            $selectedTerm = isset($_GET[$id_name]) ? $_GET[$id_name] : "";
                            if (!empty($terms)) {
                ...
                <div class="col-lg-6 col-md-6">
                    <?php 
                        $tittle_placeholder = "Select Fees Status";
                        $id_name = "fee_status";
                    ?>
                    <h6><?= $tittle_placeholder; ?></h6>
                    <?php
                        ...
                        $get_string = "fee-statuses/statuses";
                        $feeStatuses = $ccApi->school($get_string, function($classes) use($input_value)
                        {
                            $at = '';
                            foreach ($classes as $key => $class) {
                                $status = @((int)$input_value == (int)$class['pddt_id']) ? "selected": "";
                                $at .= '<option value="'.$class['pddt_id'].'" '.@$status.'>'.$class['pddt_status'].'</option>';
                            }
                            return @$at;
                        }, $debug = false);
