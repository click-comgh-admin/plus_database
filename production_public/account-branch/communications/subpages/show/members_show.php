<?php
    ...
    $get_string = "client-membership/branch/communications/one_from_branch?account_id=$account_id&message_id=".$_GET['show'];
    ...
    <?php die(); }
    $get_string = "client-membership/branch/communications/trail_from_branch?account_id=$account_id&message_id=".$_GET['show'];
                        ...
                        <td>
                            <?php
                                ...
                                if ((int)$from_branch === 1) {
                                    ...
                                    if ((int)$message['pdcbm_is_admin'] === 1) {
                                        $sender_info = $ccApi->user_info($type = "client", $account_id,
                                            $all="one_user", $id=$sender_id, function ($info)
                                        {
                                            return @$info[0];
                                        });
                                        $senderClient = $ccApi->user_info($type = "client", $sender_info['pdcu_account_id'],
                                            $all="one_client", $id=$sender_info['pdcu_account_id'], function ($info)
                                        {
                                            return @$info['pdc_name'];
                                        }, 0); ?>
                                        <?= @$sender_info['pdcu_firstname']." ".$sender_info['pdcu_surname']; ?> <i>"ADMIN"</i> of <b><?= $senderClient; ?></b><?php
                                    } else {
                                        ...
                                        ?>
                                            <button type="button" class="btn btn-success btn-sm" type="button">
...
<div class="container" email-tabs="container-mail">
    ...
    <div>
        <h5 class="text-center">
            Message Thread
            <a href="<?= Current_BaseUrl; ?>/branch-messages?<?= $_SERVER['QUERY_STRING']; ?>&reply" type="button" class="btn btn-sm bg-grow-early text-light font-weight-bold rounded-pill shadow" 
    ...
    <?php
        foreach ($message_trail as $mess_key => $message) {
            ...
            <!-- <pre><?php //print_r($message); ?></pre> -->
            <div class="mb-3 <?= ($from_branch == 1) ? "pr-4" : "pl-4"; ?>" id="<?= $message['pdccmc_id']; ?>">
