            ...
            <div class="tab-content">
                <?php
                    ...
                    <?php } else {
                        ...
                        function due_records($account_id, $ccApi, $get_year, $get_date, $get_payment_method, $get_payment_status, $payment_interval)
                        {
                            $get_string = @"records?account_id=$account_id&due_id=".$_GET['dues']."&year=$get_year&date=$get_date&paid_by=$get_payment_method&status=$get_payment_status&interval=$payment_interval";
                        ...
                        $due_records = due_records($account_id, $ccApi, $get_year, $get_date, "all", $get_payment_status, $type['pddt_payment_intervals']);
                        ...
                        foreach ($all_members as $key => $am) {
                            if (!in_array($am['pdm_id'], $DRS)) {
                                if (!in_array($am, $unpaid)) {
                                    array_push($unpaid, $am);
                                }
                            }
                        }

                        $due_records = due_records($account_id, $ccApi, $get_year, $get_date, "all", 3, $type['pddt_payment_intervals']);
                        $DRS = [];
                        foreach ($due_records as $due_record_key => $due_record) {
                            if (!in_array($due_record['pddt_member_id'], $DRS)) {
                                array_push($DRS, $due_record['pddt_member_id']);
                        ...
                        <?php } else { ?>
                            <h4 class="text-muted card-title mb-0">
                                <b>Year:</b> <?= $get_year; ?> | <b>Date:</b> <?= $get_date; ?> | <b>Payment Status:</b> Unpaid
                            </h4><hr class="my-2"> <?php
                            $message_members = $unpaid;
                            foreach ($message_types as $ms_key => $ms) { 
                                if ($ms['pdccmc_id'] != 1) {
                                    $active = ($ms_key == 1) ? "active" : "";
